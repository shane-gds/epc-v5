...
def test_python_and_duckdb_stable_keys_match() -> None:
    ...
    expected = stable_sha256("epc-v5.test", "v1", fields)
    expressions = ["null"] + [sql_literal(value) for value in fields[1:]]

    actual = (
        duckdb.connect()
        .execute(f"select {stable_sha256_sql('epc-v5.test', 'v1', expressions)}")
        .fetchone()[0]
    )
...
def test_stable_key_distinguishes_null_empty_position_and_namespace() -> None:
    base = stable_sha256("epc-v5.test", "v1", [None, ""])

    assert base != stable_sha256("epc-v5.test", "v1", ["", None])
    assert base != stable_sha256("epc-v5.other", "v1", [None, ""])
    assert base != stable_sha256("epc-v5.test", "v2", [None, ""])
